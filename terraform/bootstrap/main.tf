...
locals {
  sa_service_name    = "continuous_monitoring-cicd-deployer"
...
locals {
  developer_map = { for username, id in module.mgmt_space.developer_role_ids : "module.mgmt_space.cloudfoundry_space_role.developers[\"${username}\"]" => id }
  manager_map   = { for username, id in module.mgmt_space.manager_role_ids : "module.mgmt_space.cloudfoundry_space_role.managers[\"${username}\"]" => id }
  ...
  recreate_state_template = templatefile("${path.module}/templates/recreate_state.sh.tftpl", { import_map = local.import_map })
}
resource "local_file" "recreate_script" {
  content         = local.recreate_state_template
  filename        = "${path.module}/recreate_state.sh"
